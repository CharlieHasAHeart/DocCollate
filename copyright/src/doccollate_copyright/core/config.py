...
def load_config(path: str) -> AppConfig:
    ...
    skeleton_model = _read_env("DOCCOLLATE_LLM_MODEL_SKELETON") or str(llm.get("skeleton_model", "")).strip()
    final_model = _read_env("DOCCOLLATE_LLM_MODEL_FINAL") or str(llm.get("final_model", "")).strip()
    ...
    contact_info_raw = _read_env("DOCCOLLATE_CONTACT_INFO") or str(
        doccollate.get(
            "contact_info",
            "src/doccollate_copyright/resources/soft_copyright.yaml",
        )
    ).strip()
